...
def exportar_data_pura_al30(dias=1000):
    token = obtener_token()
    if not token:
    ...
    url = f"https://api.invertironline.com/api/v2/bCBA/Titulos/AL30/Cotizacion/seriehistorica/{fecha_desde}/{fecha_hasta}/sinAjustar"
    headers = {"Authorization": f"Bearer {token}"}

    print(f"⏳ Descargando data pura de mercado de AL30 ({dias} días)...")
...
if __name__ == "__main__":
    exportar_data_pura_al30(dias=1000)